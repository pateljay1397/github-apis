...
export const Simple3dWidget = () => {
  ...
  const _setGeometry = () => {
    ...
    if (shape === "Cone") {
      const cone = GeometryBuilder.createCone(coneHeight, coneLowerRadius, coneUpperRadius);
      if (cone)
        builder.addCone(cone);
    } else if (shape === "Sphere") {
      const sphere = GeometryBuilder.createSphere(sphereRadius);
      if (sphere)
        builder.addSphere(sphere);
    } else if (shape === "Box") {
      const box = GeometryBuilder.createBox(boxLength, boxWidth, boxHeight);
      if (box)
        builder.addBox(box);
    } else if (shape === "Torus Pipe") {
      const torusPipe = GeometryBuilder.createTorusPipe(torusPipeOuterRadius, torusPipeInnerRadius, torusPipeSweep);
      if (torusPipe)
