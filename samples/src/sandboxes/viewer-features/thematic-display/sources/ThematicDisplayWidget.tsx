...
import { Alert, Label, LabeledSelect, SelectOption, Slider, ToggleSwitch } from "@itwin/itwinui-react";
...
const ThematicDisplayWidget = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeThematicUpdate = viewport.view.displayStyle.settings.onThematicChanged.addListener((newThematic) => {
    ...
    const unsubscribeViewFlagUpdate = viewport.view.displayStyle.settings.onViewFlagsChanged.addListener((newFlags) => {
      const newProps = ThematicDisplayApi.getThematicDisplayProps(vp);
      updateState(vp, newProps, newFlags.thematicDisplay, newFlags.backgroundMap);
    });
    return () => {
      unsubscribeThematicUpdate();
      unsubscribeViewFlagUpdate();
    };
      ...
      <div className="sample-grid">
        ...
        <div className="subgrid">
          <div className="sample-grid-selects">
            <LabeledSelect label="Display Mode" displayStyle="inline" size="small" onChange={_onChangeDisplayMode} value={displayModeState} options={displayModeOptions} />
            <LabeledSelect label="Color Scheme" displayStyle="inline" size="small" onChange={_onChangeColorScheme} value={colorSchemeState} options={colorSchemeOptions} />
            <LabeledSelect label="Gradient Mode" displayStyle="inline" size="small" onChange={_onChangeGradientMode} value={gradientModeState} options={gradientModeOptions} />
          </div>
