...
export default class CrossProbingApi {
  ...
  private static elementSelected = async (ev: SelectionSetEvent) => {
    ...
    if (CrossProbingApi.lastElementSelected === sourceElementId) return;
    CrossProbingApi.lastElementSelected = sourceElementId;
    ...
    if (sourceVp?.view.is2d()) {
      targetLink = CrossProbingApi.elementMap!.filter((link: any) => link.drawElementId === sourceElementId);
      if (targetLink.length > 0) {
        const targetElement = targetLink[0].physElementId;
        // if target 3D element found, zoom into it.
        await CrossProbingApi._get3DViewport().zoomToElements(targetElement, { animateFrustumChange: true });
        ev.set.iModel.hilited.elements.addId(targetElement);
      }
    }
  ...
  private static _get3DViewport(): Viewport {
    ...
    throw new Error("No viewport with 3D model found!");
  }
  ...
  private static _get2DViewport(): Viewport {
    ...
    throw new Error("No viewport with 2D model found!");
  }
