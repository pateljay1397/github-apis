...
export const JsonViewerWidget = ({ json, onSaveJsonViewClick }: JsonViewerWidgetProps) => {
  ...
  const _handleJsonTextChange = async (event: React.ChangeEvent<HTMLTextAreaElement>) => {
    try {
      ...
      setJsonValueState(event.target.value);
      setJsonErrorState(error as any);
    }
  };
  ...
  const showError = (stateProp: string | undefined) => {
    ...
    return (
      <div style={{ overflowWrap: "break-word" }}>
        <Small style={{ color: "var(--foreground-alert)" }}>${stateProp}</Small>
      </div>
    );
  };
