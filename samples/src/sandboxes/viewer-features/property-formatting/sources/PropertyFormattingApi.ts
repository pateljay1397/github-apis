...
interface OverlySimplePropertyRecord {
  name: string;
  displayLabel: string;
  displayValue: string;
}
...
export class PropertyFormattingApi {
  ...
  public static createTableDataProvider(keys: KeySet, imodel: IModelConnection) {
    const dataProvider = new PresentationTableDataProvider({ imodel, ruleset: DEFAULT_PROPERTY_GRID_RULESET, pageSize: 10 });
    dataProvider.keys = keys;
    return dataProvider;
  }
  ...
  https://github.com/iTwin/itwinjs-core/blob/master/presentation/components/src/presentation-components/common/ContentBuilder.ts
