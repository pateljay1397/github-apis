...
export interface ExplodeObject extends SelectOption<string> {
  elementIds: string[];
  categories?: string[];
}
...
export default class ExplodeApi {
  ...
  public static isolateElements(vp: Viewport, elementIds: string[]) {
    ...
    provider.isolateElements(elementIds, vp, true);
  }
  ...
  /** Uses the iTwin.js tools to fit the view to the object on screen. */
...
export class ExplodeProvider implements TiledGraphicsProvider, FeatureOverrideProvider {
  ...
  public constructor(public vp: Viewport) {
    ...
    ExplodeApi.cleanUpCallbacks.push(removeListener);  // This will insure the listener is removed before swapping samples.
    this.explodeTileTreeRef = new ExplodeTreeReference(vp.iModel);
  }
