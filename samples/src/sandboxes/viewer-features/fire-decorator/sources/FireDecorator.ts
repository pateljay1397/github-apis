...
import flameImage from "./particle-gradient-flame.png";
import smokeImage from "./particle-gradient-smoke.png";
...
export class FireEmitter implements Decorator {
  ...
  private static async tryTextures(): Promise<boolean> {
    if (!FireEmitter._fireTexture)
      FireEmitter._fireTexture = await FireDecorationApi.allocateTextureFromUrl(flameImage);
    if (!FireEmitter._smokeTexture)
      FireEmitter._smokeTexture = await FireDecorationApi.allocateTextureFromUrl(smokeImage);
